...
namespace VoxelQuadsViewer
{
    ...
    class CubeWindow : GameWindow
    {
        ...
        protected override void OnLoad()
        {
            ...
            //        // Ambient (so nothing is fully black)
            //        float ambientStrength = 0.4;
            //        vec3 ambient = ambientStrength * lightColor;

            //        // Diffuse (smooth shading)
            ...
            //_flatShader = GL.CreateProgram();
            //int flatVSId = GL.CreateShader(ShaderType.VertexShader);
            //GL.ShaderSource(flatVSId, flatVS);
            //GL.CompileShader(flatVSId);
            //int flatFSId = GL.CreateShader(ShaderType.FragmentShader);
            //GL.ShaderSource(flatFSId, flatFS);
            //GL.CompileShader(flatFSId);
            //GL.AttachShader(_flatShader, flatVSId);
            //GL.AttachShader(_flatShader, flatFSId);
            //GL.LinkProgram(_flatShader);
            //GL.DeleteShader(flatVSId);
            //GL.DeleteShader(flatFSId);
